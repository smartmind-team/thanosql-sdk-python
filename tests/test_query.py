...
plain_query_template_name = f"test_plain_{fake.unique.pystr(min_chars=8, max_chars=8).lower()}"
basic_query_template_name = f"test_basic_{fake.unique.pystr(min_chars=8, max_chars=8).lower()}"
changed_query_template_name = f"test_changed_{fake.unique.pystr(min_chars=8, max_chars=8).lower()}"
...
query_test_table_name = f"test_query_{fake.unique.pystr(min_chars=8, max_chars=8).lower()}"
...
def test_create_query_template_success(client: ThanoSQL):
    ...
    # save to db
    res = client.query.template.create(name=plain_query_template_name, query=plain_query_template_string)
    assert {"id", "name", "query", "parameters", "created_at", "updated_at"} == set(res["query_template"])
    ...
    res = client.query.template.create(name=basic_query_template_name, query=basic_query_template_string)
    ...
    with pytest.raises(ThanoSQLAlreadyExistsError):
        client.query.template.create(name=plain_query_template_name, query=plain_query_template_string)
    
    with pytest.raises(ThanoSQLAlreadyExistsError):
        client.query.template.create(name=basic_query_template_name, query=basic_query_template_string)
...
def test_update_query_template_invalid(client: ThanoSQL):
    # not found
    with pytest.raises(ThanoSQLNotFoundError):
        client.query.template.update(current_name=fake.unique.pystr(min_chars=8, max_chars=10))
    
    # new name too long
    with pytest.raises(ThanoSQLValueError):
        client.query.template.update(current_name=basic_query_template_name, new_name=fake.unique.pystr(min_chars=30, max_chars=35))
    
    # new name already used
    with pytest.raises(ThanoSQLAlreadyExistsError):
        client.query.template.update(current_name=basic_query_template_name, new_name=plain_query_template_name)
    
    # invalid jinja template
    with pytest.raises(ThanoSQLValueError):
        client.query.template.update(current_name=basic_query_template_name, query=invalid_query_template_string)


def test_update_query_template_success(client: ThanoSQL):
    # update name only
    # make sure name is changed but the rest stays the same
    res = client.query.template.update(current_name=plain_query_template_name, new_name=changed_query_template_name)
    ...
    res = client.query.template.update(current_name=basic_query_template_name, query=changed_query_template_string)
...
def test_post_query_invalid(client: ThanoSQL, basic_table_name: str):
    ...
    with pytest.raises(ThanoSQLNotFoundError):
        client.query.execute(schema=fake.unique.pystr(8), table_name=fake.unique.pystr(8), query=plain_query_template_string)
    
    # table already exists
    with pytest.raises(ThanoSQLAlreadyExistsError):
        client.query.execute(table_name=basic_table_name, query=plain_query_template_string)
...
def test_post_query_success(client: ThanoSQL, basic_table_name, empty_table_name):
    ...
    res = client.query.execute(query=plain_query_template_string, table_name=query_test_table_name)
    assert set(res.keys()) == query_log_keys
    assert res["error_result"] is None
    ...
    res = client.query.execute(template_name=basic_query_template_name, parameters=params, table_name=empty_table_name, overwrite=True)
    assert set(res.keys()) == query_log_keys
    assert res["error_result"] is None
    assert res["destination_schema"] == "public"
    assert res["query"] == completed_changed_query
...
@pytest.mark.parametrize("name", [changed_query_template_name, basic_query_template_name])
