...
from tests.utils.table import create_table, create_view
...
def basic_table(client: ThanoSQL) -> TableServiceObject:
    table_object = TableObject(columns=[BaseColumn(type=col[0], name=col[1]) for col in basic_table_columns])
...
def empty_view(client: ThanoSQL, empty_table_name: str) -> dict:
    ...
    return create_view(client=client, name=name, column_names="*", table_name=empty_table_name)


@pytest.fixture(scope="module")
def basic_view(client: ThanoSQL, basic_table_name: str) -> dict:
    name = f"test_basic_view_{fake.unique.pystr(8).lower()}"
    return create_view(client=client, name=name, column_names=basic_view_columns, table_name=basic_table_name)
