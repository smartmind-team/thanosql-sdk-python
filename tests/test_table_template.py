...
def test_create_table_template_invalid(client: ThanoSQL, empty_table_template: TableTemplate):
    ...
    with pytest.raises(ThanoSQLValueError):
        client.table.template.create(
            name=fake.unique.pystr(min_chars=35, max_chars=40).lower(),
    ...
    with pytest.raises(ThanoSQLValueError):
        client.table.template.create(
            name=name, table_template=TableObject(), version="2.0.0"
        )


def test_create_table_template_success(
    client: ThanoSQL, empty_table_template_name: str
):
...
def test_delete_table_template_success(
    client: ThanoSQL, empty_table_template_name: str
):
    # delete a specific version only
    res = client.table.template.delete(
        name=empty_table_template_name, version=latest_version
    )
    assert res["table_template_name"] == empty_table_template_name

    # now said version should not be found
    with pytest.raises(ThanoSQLNotFoundError):
        client.table.template.get(
            name=empty_table_template_name, version=latest_version
        )
